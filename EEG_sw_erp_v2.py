...
for file in files :
    ...
    classtype = id_recording.type.loc[
        id_recording['date'] == recording_date
        ].iloc[0]
    
    print(f"\n... CURRENTLY PROCESSING {sub_id} ...")
    if len(glob.glob(
            f"{fig_path}/slowwaves/{sub_id}_{recording_date}_270623.npy")
            ) < 1 :
        print(f"No SW files found for {sub_id} at {recording_date}")
        continue
    ...
    report = mne.Report(
        title=f"Epocheds SW of : {sub_id}_{recording_date}")
    ...
    report.save(
        f"{fig_path}/Reports/SW_ERP_{sub_id}_{recording_date}_figure.html", 
        overwrite=True,
        open_browser = False)  
    ...
    # Saving epochs
    epochs_savename = f"{preproc_path}/erp_slowwaves_{sub_id}_{recording_date}_{todaydate}_epo.fif"
    epochs_clean.save(
        epochs_savename,
        overwrite = True
        )
