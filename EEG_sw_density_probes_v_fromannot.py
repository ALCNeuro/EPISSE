...
for file in glob.glob(f"{preproc_path}/erp_slowwaves*270623*_epo.fif") :
    sub_id = file[len(preproc_path):][15:-22]
    recording_date = file[len(preproc_path):][-21:-15]
    ...
    #     )
    epochs.annotations.delete(
        np.where(
            (epochs.annotations.description == 'BAD boundary') 
            | (epochs.annotations.description == 'EDGE boundary')
            )[0]
        )
    
    df_annot = epochs.annotations.to_data_frame()
    ...
    You have the annotations and onset from the df_annot
    -> You need to compute the onset in second by doing
    -> onset - epochs.info['meas_date']
        => Make sure that it's not shifted because of the cropping
    -> And you might have an approximately accurate sw density
